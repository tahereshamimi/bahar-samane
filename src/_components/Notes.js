...
class Notes extends Component {
  ...
  addNote = e => {
    this.setState({
      visibility: e.target.value === "" ? false : true,
      note: e.target.value
    });
  };
    ...
    return (
      <div>
        <Container>
          <Row className="justify-content-center">
            <form onSubmit={this.onSubmit} className="input-card p-3">
              <div>
                <span
                  className={this.state.importanceButton}
                  onClick={this.changeImportance}
                >
                  {this.state.importance ? "urgent" : "normal"}
                </span>
              </div>
              <div>
                <textarea
          ...
          <Row>
            {this.state.notes.map((note, index) => (
              <Col md="6" key={uuidv1()}>
                <div
                  className="card-style position-relative shadow"
                  onClick={event => this.deleteNote(index, event)}
                >
                  {note.note}
                  {note.importance ? (
                    <span className="position-absolute star ">*</span>
                  ) : null}
                </div>
              </Col>
            ))}
        ...
        >
          ...
          <ModalFooter>
            <Button color="success" onClick={this.confirmDelete}>
              Delete
            </Button>
